...
<header class="page-header" data-background="{{ asset('front/images/page-header-bg.jpg') }}" data-stellar-background-ratio="0.7">
	<div class="container">
	<h2>Actualités</h2>
		<p>Découvrez nos actualités et nouvelles</p>
	</div>
	<!-- end container -->
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-8">
		  ...
		  <div class="blog-post paginated-item">
		  <figure><img src="{{ asset('storage/'.$item->image) }}" alt="Image"></figure>
			  <div class="post-content">
				  <span class="post-date">{{ $item->created_at->format('d M,Y') }}</span>
			  <h3 class="post-title">{{ $item->titre }}</h3>
                    @php
                    $texteBrut1 = strip_tags($item->contenu);
                @endphp
				  <p>{{ substr($texteBrut1, 0, 100) }}... </p>
                <button class="btn btn-success  mb-2 me-4 btn-sm">
                    <b><a href="{{ url('actualite/'.$item->slug) }}">En savoir plus</a></b>
                </button>
			  </div>
			  <!-- end post-content -->
		  </div>
      ...
      <div class="col-lg-4">
			...
			<div class="widget">
				...
				<form method="POST" action="{{ route('front.actualite.search') }}">
                    @csrf
				...
				<ul class="categories">
                    ...
                    <li><a href="{{ url('actualite/recherche_par_category/'.$category->id) }}">{{ $category->nom }}</a></li>
