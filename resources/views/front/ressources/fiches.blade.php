    ...
    <div class="row">
      ...
      <figure><img src="{{ asset('storage/'.$item->image) }}" alt="Image"></figure>
        ...
        <h3>{{ substr($item->titre, 0, 50) }}...</h3>
          ...
          <button class="btn btn-success  mb-2 me-4 btn-sm"><b><a href="{{ url('ressources/fiches-expériences/'.$item->slug) }}">En savoir plus</a></b></button>
        </div>
        <!-- end content -->
      </div>
      <!-- end side-image-box -->
      ...
      <div class="widget">
        ...
        <form method="POST" action="{{ route('ressources.fiches-expériences.search') }}">
            @csrf
        ...
        @if ($categories->isNotEmpty())
            ...
            <li><a href="{{ url('ressources/fiches-expériences/recherche_par_category/'.$category->id) }}">{{ $category->nom }}</a></li>
