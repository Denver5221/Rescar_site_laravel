...
<section class="content-section">
  <div class="container">
    <div class="row">
      <div class="col-lg-8">
      <div class="blog-post">
      <figure><img src="{{ asset('storage/'.$etudes->image) }}" alt="Image"></figure>
        <div class="post-content">
          <span class="post-date">De {{ $etudes->created_at->format('d M, Y') }}</span>
          @php
                $tags = $etudes->tags;
          ...
          <p>Categories :
            @if ($etudes->categories->isNotEmpty())
                @foreach ($etudes->categories as $categorie)
        ...
        <h3 class="post-title">{{ $etudes->titre }}</h3>
         {{-- <div class="author"> <img src="{{ asset('storage/'.$etudes->image) }}" alt="Image"> <span>par <strong>Rescar-aoc</strong></span> </div> --}}
            <!-- end author -->
            @php
        ...
        <blockquote>{{ $etudes->titre }}        </blockquote>
        <p>{{ substr($etudes->description, strlen($paragraphs[0])) }}</p>
<br>
        <center>
                  <h5>Fichiers</h5>
        ...
        @if ($etudes->file_fr)
          <li class="list-group-item d-flex justify-content-between align-items-center">
            ...
            <span class="rounded-pill">
                <a href="{{ asset('storage/'.$etudes->file_fr) }}" target="_blank">
        ...
        @endif
          ...
          <li class="list-group-item d-flex justify-content-between align-items-center">
            Fichier (Anglais)
            <span class="rounded-pill">
                <a href="{{ asset('storage/'.$etudes->file_an) }}" target="_blank">
              <button class="btn btn-success  mb-2 me-4">
                <svg style="color: white" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-download" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z" fill="white"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z" fill="white"></path> </svg>
                <span class="btn-text-inner">Telecharger</span>
              </button></a>
          ...
          <li class="list-group-item d-flex justify-content-between align-items-center">
            Fichier (Portugal)
             <span class="rounded-pill">
                <a href="{{ asset('storage/'.$etudes->file_pr) }}" target="_blank">
               <button class="btn btn-success  mb-2 me-4">
                 <svg style="color: white" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-download" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z" fill="white"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z" fill="white"></path> </svg>
                 <span class="btn-text-inner">Telecharger</span>
               </button></a>
        ...
        <div class="services-list-boxedsingle">
            ...
            <li>
              ...
              <div class="content">
                ...
                @if ($etudes->active_commentaire == 1)
    ...
    <div class="modal fade" id="ajouterRoleModal{{ $comment->id }}" tabindex="-1" role="dialog" aria-labelledby="ajouterRoleModalLabel{{ $comment->id }}" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('ressources.etudes-publications.commentaire') }}" method="POST">
                        @csrf
    ...
    @endif
        ...
        <div class="blog-post">
          <div class="col-lg-12">
            ...
            <form method="post" action="{{ route('ressources.etudes-publications.commentaire') }}">
                @csrf
            <textarea name="content"> </textarea>
            <input type="hidden" name="id_lier" value="{{ $etudes->id }}">
      ...
      <div class="widget">
        ...
        <form method="POST" action="{{ route('ressources.etudes-publications.search') }}">
            @csrf
        ...
        <ul class="categories">
            ...
            <li><a href="{{ url('ressources/etudes/recherche_par_category/'.$category->id) }}">{{ $category->nom }}</a></li>
...
<!-- Votre code HTML existant (bouton "repondreBtn" et autres éléments) -->
    ...
    document.getElementById('repondreBtn').addEventListener('click', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Envoie des données du formulaire via AJAX à la route Laravel définie pour gérer la réponse
                axios.post('/commentaires/repondre', {
                    reponseMessage: reponse,
                    id_lier: $etudes->id, // Remplacez cette valeur par l'ID réel de la relation concernée
                    parent_id:  // Remplacez cette valeur par l'ID réel du commentaire parent (s'il y a lieu)
                })
                .then((response) => {
                    // Traitement de la réponse du serveur si nécessaire
                    console.log(response.data);
                    Swal.fire('Succès', 'Réponse enregistrée avec succès!', 'success');
                })
                .catch((error) => {
                    // Gestion des erreurs si nécessaire
                    console.error(error);
                    Swal.fire('Erreur', 'Erreur lors de l\'enregistrement de la réponse!', 'error');
                });
            }
        });
    });
...
{{-- <script>
        document.getElementById('repondreBtn').addEventListener('click', function() {
            // Formulaire HTML à afficher dans la popup
            const formHtml = `
               <form id="reponseForm" method="post" action="{{ route('ressources.etudes-publications.commentaire') }}">
                @csrf
                    <label for="reponseMessage">Votre réponse :</label>
                    <textarea id="reponseMessage" name="content" rows="4" cols="50"></textarea>
                </form>
            `;
